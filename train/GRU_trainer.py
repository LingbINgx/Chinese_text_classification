...
from models.textrnn_classifier_GRU import TextRNNGRUClassifier
...
@dataclass
class TrainConfig_textGRU(BaseConfig):
    train_file_path: str
    val_file_path: str
    test_file_path: str
    max_length: int
...
@save_logger
@logger.catch
def train(config_path: str | Path = "params/params_GRU.yaml"):
    logger.info(f"running {Path(__file__).name} with config: {config_path}")

    root_dir = Path(__file__).resolve().parents[1]
    config = TrainConfig_textGRU.from_yaml(root_dir / config_path)
    ...
    best_model_path = output_dir / "textGRU.pt"
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for batch in tqdm(train_loader, desc=f"TextRNN Epoch {epoch}"):
    ...
    with open(output_dir / "textGRU_label_mapping.json", "w", encoding="utf-8") as file:
        json.dump({"label2id": label2id, "id2label": id2label}, file, ensure_ascii=False, indent=2)

    with open(output_dir / "textGRU_config_snapshot.json", "w", encoding="utf-8") as file:
        json.dump(config.__dict__, file, ensure_ascii=False, indent=2)

    logger.info(f"TextRNN_GRU 训练完成，模型已保存到: {output_dir}")
