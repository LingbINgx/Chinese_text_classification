...
from models.textrnn_classifier import TextRNNClassifier
...
@dataclass
class TrainConfig_textrnn(BaseConfig):
    train_file_path: str
    val_file_path: str
    test_file_path: str
    max_length: int
...
@save_logger
@logger.catch
def train(config_path: str | Path = "params/params_textrnn.yaml"):
    logger.info(f"running {Path(__file__).name} with config: {config_path}")

    root_dir = Path(__file__).resolve().parents[1]
    config = TrainConfig_textrnn.from_yaml(root_dir / config_path)
